...
_IRQL_requires_max_(PASSIVE_LEVEL) static bool VmpIsDeviceMonInstalled();
...
#pragma alloc_text(PAGE, VmpIsDeviceMonInstalled)
...
_Use_decl_annotations_ static SharedProcessorData *VmpInitializeSharedData() {
  ...
  if (!shared_data->msr_bitmap) {
    ExFreePoolWithTag(shared_data, kDeviceMonCommonPoolTag);
    return nullptr;
  }
  ...
  if (!io_bitmaps) {
    ExFreePoolWithTag(shared_data->msr_bitmap, kDeviceMonCommonPoolTag);
    ExFreePoolWithTag(shared_data, kDeviceMonCommonPoolTag);
    return nullptr;
  }
...
_Use_decl_annotations_ static UCHAR *VmpBuildIoBitmaps() {
  ...
  const auto io_bitmaps = static_cast<UCHAR *>(ExAllocatePoolWithTag(
      NonPagedPool, PAGE_SIZE * 2, kDeviceMonCommonPoolTag));
  if (!io_bitmaps) {
    return nullptr;
  }
...
_Use_decl_annotations_ static NTSTATUS VmpStartVm(void *context) {
  ...
  NT_ASSERT(VmpIsDeviceMonInstalled() == ok);
  if (!ok) {
...
_Use_decl_annotations_ static void VmpInitializeVm(
  ...
  const auto processor_data =
      static_cast<ProcessorData *>(ExAllocatePoolWithTag(
          NonPagedPool, sizeof(ProcessorData), kDeviceMonCommonPoolTag));
  if (!processor_data) {
    return;
  }
  ...
  processor_data->vmcs_region =
      static_cast<VmControlStructure *>(ExAllocatePoolWithTag(
          NonPagedPool, kVmxMaxVmcsSize, kDeviceMonCommonPoolTag));
...
_Use_decl_annotations_ void VmTermination() {
  ...
  NT_ASSERT(!VmpIsDeviceMonInstalled());
}
...
_Use_decl_annotations_ static void VmpFreeProcessorData(
  ...
  if (processor_data->vmcs_region) {
    ExFreePoolWithTag(processor_data->vmcs_region, kDeviceMonCommonPoolTag);
  }
  if (processor_data->vmxon_region) {
  ...
  ExFreePoolWithTag(processor_data, kDeviceMonCommonPoolTag);
}
...
_Use_decl_annotations_ static void VmpFreeSharedData(
  ...
  ExFreePoolWithTag(processor_data->shared_data, kDeviceMonCommonPoolTag);
}

// Tests if DeviceMon is already installed
_Use_decl_annotations_ static bool VmpIsDeviceMonInstalled() {
  PAGED_CODE()
