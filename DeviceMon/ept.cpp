...
EptpMapGpaToHpa(
	...
	_In_ EptData *	ept_data)
{
	return EptpConstructTables(ept_data->ept_pml4, 4, physical_address, ept_data);
}
...
_Use_decl_annotations_ EptData *EptInitialization() {
  ...
  const auto ept_data = static_cast<EptData *>(ExAllocatePoolWithTag(
      NonPagedPool, sizeof(EptData), kDeviceMonCommonPoolTag));
  if (!ept_data) {
    return nullptr;
  }
  ...
  const auto ept_pml4 = static_cast<EptCommonEntry *>(ExAllocatePoolWithTag(
      NonPagedPool, PAGE_SIZE, kDeviceMonCommonPoolTag));
  if (!ept_pml4) {
    ExFreePoolWithTag(ept_data, kDeviceMonCommonPoolTag);
    return nullptr;
  }
...
_Use_decl_annotations_ static EptCommonEntry *EptpAllocateEptEntryFromPool() {
  ...
  const auto entry = static_cast<EptCommonEntry *>(ExAllocatePoolWithTag(
      NonPagedPool, kAllocSize, kDeviceMonCommonPoolTag));
  if (!entry) {
    return entry;
  }
...
_Use_decl_annotations_ void EptHandleEptViolation(EptData *ept_data) {
  ...
  auto ept_entry = EptGetEptPtEntry(ept_data, fault_pa);
  ...
  ept_entry = EptpConstructTables(ept_data->ept_pml4, 4, fault_pa, ept_data);
...
#pragma warning(disable : 6001)
    ExFreePoolWithTag(preallocated_entries[i], kDeviceMonCommonPoolTag);
#pragma warning(pop)
  }
  ExFreePoolWithTag(preallocated_entries, kDeviceMonCommonPoolTag);
}
...
_Use_decl_annotations_ static void EptpDestructTables(EptCommonEntry *table,
    ...
    if (entry.fields.physial_address) {
      ...
      switch (table_level) {
        ...
        case 2:  // table == PDT, sub_table == PT
          ExFreePoolWithTag(sub_table, kDeviceMonCommonPoolTag);
          break;
        default:
          HYPERPLATFORM_COMMON_DBG_BREAK();
          break;
  ...
  ExFreePoolWithTag(table, kDeviceMonCommonPoolTag);
}
}  // extern "C"
